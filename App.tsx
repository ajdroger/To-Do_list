...
import { Plus, ListTodo, HelpCircle, Layout, Calendar, CheckCircle2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-indigo-100 pb-28">
      
      {/* Header Moderno */}
      <header className="bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-200/60 safe-top transition-all">
        <div className="max-w-2xl mx-auto px-5 py-4">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-slate-500 text-sm font-medium mb-0.5">{getGreeting()},</p>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Le tue Attività</h1>
            </div>
            <button 
              onClick={() => setView('tutorial')}
              className="group p-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-indigo-50 hover:text-indigo-600 transition-all active:scale-95"
          ...
          {tasks.length > 0 && (
            <div className="mb-5 flex items-center gap-3">
              ...
              <span className="text-xs font-bold text-slate-500 w-9 text-right">{progress}%</span>
            </div>
          )}
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        {filteredTasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
            <div className="w-24 h-24 bg-gradient-to-br from-indigo-50 to-slate-100 rounded-3xl flex items-center justify-center mb-6 shadow-inner text-indigo-300">
      ...
      <div className="fixed bottom-8 right-6 z-20">
        <button
          ...
          className="group relative bg-slate-900 hover:bg-indigo-600 text-white w-16 h-16 rounded-2xl shadow-2xl shadow-slate-400/50 flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-90"
        >
